// Focused decor lineup harness (not part of the suite). Usage: node test/decorshot.js [hour]
...
(async () => {
  ...
  const box = await page.evaluate((hour) => {
    ...
    // ground props spread across the street, plus a lit lamp and strung bunting
    banco(6, GROUND); vaso(34, GROUND); jardineira(52, GROUND); cadeira(80, GROUND);
    lixeira(98, GROUND); varal(118, GROUND, 26, 21, 3); posteLuz(160, GROUND, true);
    bandeirinhas(60, GROUND - 30);
    return { x: r.left, y: r.top, w: r.width, h: r.height };
  }, hour);
  await page.waitForTimeout(120);
  const tag = hour === 12 ? '' : '-' + hour;
  await page.screenshot({ path: path.resolve(__dirname, '..', 'shot-decor' + tag + '.png'),
    clip: { x: Math.max(0, box.x), y: Math.max(0, box.y), width: box.w, height: box.h * 0.86 } });
  console.log('wrote shot-decor' + tag + '.png');
  await browser.close();
})();
